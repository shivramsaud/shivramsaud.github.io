(function(){
  ...
  var na=document.querySelectorAll('.na')
  var ss=document.querySelectorAll('section[id]')
  function act(){
    var sy=window.scrollY+70,cur=''
  ...
  var projects=[
    {title:'Semantic Video Retrieval',sem:'6th Sem',
     desc:'CLIP + FAISS text-to-video search. Encodes frames as 512-dim embeddings for natural-language shot retrieval.',
     tags:['CLIP','FAISS','PyTorch'],url:'https://github.com/shivramsaud/video-retrieval'},
    {title:'Invasive Plant Classification',sem:'5th Sem',
     desc:'27-class classifier for invasive plants in Nepal using YOLOv12, ResNet, DenseNet, ConvNeXtV2 on GBIF data.',
    ...
    {title:'Hindi-Nepali Machine Translation',sem:'4th Sem',
     desc:'Fairseq transformer with VecMap alignment & WSD. 20k parallel corpus, evaluated with BLEU/chrF/TER.',
    ...
    {title:'IoT Environmental Monitor',sem:'3rd Sem',
     desc:'DHT22/CO2 sensors on Arduino, RPi edge gateway with SQLite, Pi Camera for mushroom farm monitoring.',
    ...
    {title:'Desktop Text Editor',sem:'2nd Sem',
     desc:'Multi-tab editor with syntax highlighting, find/replace, line numbers. Built in Python + CustomTkinter.',
     tags:['Python','Tkinter'],url:'https://github.com/shivramsaud/text-editor-desktop'}
  ]
  var sg=document.getElementById('sg')
  if(sg){
    projects.forEach(function(p){
      var c=document.createElement('div');c.className='pc'
      var tg=document.createElement('div');tg.className='pcg'
      p.tags.forEach(function(t){var s=document.createElement('span');s.className='pcgt';s.textContent=t;tg.appendChild(s)})
  ...
  // Publications
  var pubs=[
    {title:'Detection of Language, Hate Speech and Targets using FastText & BERT',venue:'CHiPSAL 2025',
    ...
    {title:'Fine-Grained Toxicity Detection in Online Gaming',venue:'EEUCA 2026',
  ...
  if(skg){
    groups.forEach(function(g){
      ...
      var l=document.createElement('p');l.className='skl';l.textContent=g.label;d.appendChild(l)
      var i=document.createElement('div');i.className='ski'
      g.items.forEach(function(n){var s=document.createElement('span');s.className='skit';s.textContent=n;i.appendChild(s)})
